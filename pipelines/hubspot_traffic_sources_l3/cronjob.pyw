...
from sqlops.tables.hubspot_traffic_sources_l3 import Traffic_Source_L3, get_level2_data_labels
...
previous_date = dt.today()-datetime.timedelta(days=1)
...
JOB_NAME="Traffic By Source L3"
JOB_TYPE=f"Cronjob Previous Day"
TABLENAME="powerbi_python.hubsot_traffic_sources_l3"
...
def main():
    ...
    while start_date_ <= end_date_:
        ...
        label_list = get_level2_data_labels(date=start_date_str,sql_engine=sql_engine)
        # print(label_list)
        print(f"Calling HubSpot traffic by source L3 data for date: {start_date_str}")
        for label in label_list:
            ...
            response = HubSpotSourceApi.get_total(
                                            ...
                                            drill_down_value1=label['parent_breakdown'],
                                            drill_down_value2=label['breakdown']
                                        )
            ...
            temp_data2 = SerializeHubspotResponse.add_key_value(temp_data1,{"parent_breakdown":label['parent_breakdown']})
            temp_data_final = SerializeHubspotResponse.add_key_value(temp_data2,{"sub_parent_breakdown":label['breakdown']})
    ...
    sql_obj = Traffic_Source_L3(engine=sql_engine)
...
if __name__ == '__main__':
    try:
        ...
        myLogger(level='info',msg=f'{imported_records_count} imported to hubsot_traffic_sources_l3 table',folder_path=LOG_FILE_PATH)
