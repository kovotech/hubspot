...
class Traffic_Source_L2:
    ...
    def getTable() -> Table:
        TABLE = Table(
            "hubsot_traffic_sources_l2",MetaData(),
    ...
    def create_table(self):
        table = Traffic_Source_L2.getTable()
    ...
    def map(src:dict):
        output_dict:dict = {}
        try:
            output_dict['id'] = str(src['parent_breakdown'])+"_"+(src['breakdown'])+"_"+str(src['date'])
        except:
            output_dict['id'] = None
        try:
    ...
    def get_insert_stmt(record:dict):
        payload = Traffic_Source_L2.map(record)
        table = Traffic_Source_L2.getTable()
    ...
    def import_to_sql(self,record:dict):
        with self.engine.begin() as connx:
            deleteStmt = Traffic_Source_L2.get_delete_stmt(record)
            insertStmt = Traffic_Source_L2.get_insert_stmt(record)
            connx.execute(deleteStmt)
            connx.execute(insertStmt)


def get_level1_data_labels(date:str,sql_engine:sqlalchemy.engine.Engine):
    query = f"""select breakdown from powerbi_python.hubsot_traffic_sources_l1 where date = '{date}'"""
    df = pd.read_sql(query,con=sql_engine)
    label_list = list(df['breakdown'])
    return label_list